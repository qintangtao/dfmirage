...
//品质
int H264Encoder::Init(int width,int height,int fps,int keyframe,int bitrate,int level,int qp,int nUseQP)
{
    if (keyframe<5)
    {
        keyframe=10;
    ...
    H264Encoder::X264_CONFIG_SET(&m_x264_param,level);
...
/////////////////////////////////////////////
    ...
    //int nRet = x264_picture_alloc(m_x264_picin,X264_CSP_BGRA/*X264_CSP_I422*/,m_nwidth,m_nheight);
    m_bIsworking=true;
    return 0;
}

int H264Encoder::Clean()
{
    if (m_hx264!=NULL)
    {
        m_bIsworking=false;
...
unsigned char* H264Encoder::Encoder(unsigned char *indata, int inlen, int &outlen, bool &bIsKeyFrame)
{
...
void H264Encoder::X264_CONFIG_SET(x264_param_t*param,int mode)
{
    switch (mode)
    {
    case ZH264_DEFAULT:
...
void H264Encoder::GetSPSAndPPS(unsigned char*sps,long&spslen,unsigned char*pps,long&ppslen)
{
    spslen=0;
    ppslen=0;
    ...
    if (m_hx264!=NULL)
    {
        ...
        for (int i = 0; i < i_nal; i++) {
            switch (tnal[i].i_type) {
            case NAL_SPS:
            {
                spslen=tnal[i].i_payload-4;
                memcpy(sps,tnal[i].p_payload+4,spslen);
            }
                break;
            case NAL_PPS:
            {
                ppslen=tnal[i].i_payload-4;
                memcpy(pps,tnal[i].p_payload+4,ppslen);
            }
                break;
            }
        }
